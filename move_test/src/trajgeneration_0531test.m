...
[P_r,R_r,V_r,V0_r,W_r,A_r,A0_r,Q,Q_V,Q_A,j_t] = generate_trajectory(0.1,5,0,-5*pi/25*pi/25);
...
function [P_D,R_D,V_D,V0_D,W_D,A_D,A0_D,q,Q_V,Q_A,j_t] = generate_trajectory(dt,traj_no,V_init,A_init)   
    if traj_no == 1
        q_size = 50;
        m = 10;
        q = zeros(3,q_size+1);
    ...
    elseif traj_no == 5
        ...
        for i = 1 : q_size+1
            ...
            if i == 1
              ...
              q_V(:,i) =  2*(q(:,i)-q(:,i-1))/dT-q_V(:,i-1);          
              q_A(:,i) =  2*(q_V(:,i)-q_V(:,i-1))/dT-q_A(:,i-1);
