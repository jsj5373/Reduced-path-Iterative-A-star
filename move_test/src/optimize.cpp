...
int main(){
    ...
    for (int i = 1; i < q_size+1 ;i++){
        delta_x.block(1,i,3,i) << q(1,i+1)-q(1,i)-q_V(1,i)*dT-(1/2)*q_A(1,i)*pow(dT,2), q_V(1,i+1)-q_V(1,i)-q_A(1,i)*dT, q_A(1,i+1)-q_A(1,i);
        delta_y.block(1,i,3,i) << q(2,i+1)-q(2,i)-q_V(2,i)*dT-(1/2)*q_A(2,i)*pow(dT,2), q_V(2,i+1)-q_V(2,i)-q_A(2,i)*dT, q_A(2,i+1)-q_A(2,i);
        delta_z.block(1,i,3,i) << q(3,i+1)-q(3,i)-q_V(3,i)*dT-(1/2)*q_A(3,i)*pow(dT,2), q_V(3,i+1)-q_V(3,i)-q_A(3,i)*dT, q_A(3,i+1)-q_A(3,i);
